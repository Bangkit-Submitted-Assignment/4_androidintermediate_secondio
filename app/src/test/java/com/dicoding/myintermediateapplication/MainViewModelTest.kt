...
//        val data: PagingData<ListStoryItem> = PagingData.from(dummyQuote)
        ...
        `when`(quoteRepository.getStoryPaging(token)).thenReturn(expectedQuote)

        val mainViewModel = MainViewModel(quoteRepository)
        val actualQuote: PagingData<ListStoryItem> = mainViewModel.getStoryPaging(token).getOrAwaitValue()
    ...
    fun whenGetQuoteEmptyShouldReturnNoData() = runTest {
        ...
        val expectedQuote = MutableLiveData<PagingData<ListStoryItem>>()
        expectedQuote.value = data
        Mockito.`when`(quoteRepository.getStoryPaging(token)).thenReturn(expectedQuote)
